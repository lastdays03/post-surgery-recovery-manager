...
export function TodayMealSection({ userId }: TodayMealSectionProps) {
  ...
  const formattedDate = format(new Date(), "yyyy년 M월 d일 EEEE", {
    locale: ko,
  });
  ...
  return (
    <section className="space-y-4">
      <div className="flex justify-between items-end">
        <h3 className="text-2xl font-bold text-gray-900 pt-8">
          오늘의 식단{" "}
      ...
      <div className="relative overflow-hidden min-h-[200px] rounded-2xl">
        ...
        {showContent && (
          <div
            className={`space-y-6 transition-transform duration-500 ease-in-out ${justGenerated ? "animate-in slide-in-from-right duration-500" : ""}`}
          >
            <Carousel
              opts={{
                align: "start",
            ...
            >
              <CarouselContent className="-ml-4">
                ...
                {meals
                  .filter((m) => m.mealTime.includes("snack"))
                  .map((snack, index) => (
                    <CarouselItem
                      key={snack.id || `snack-${index}`}
                      className="pl-4 basis-full sm:basis-1/2 lg:basis-1/3 xl:basis-1/4"
                    >
                      <MealCard
                        title={
                          meals.filter((m) => m.mealTime.includes("snack"))
                            .length > 1
                            ? `간식 ${index + 1}`
                            : "간식"
                        }
                        ...
                        simple
                        onClick={() =>
                          setSelectedMeal({ meal: snack, mealTime: "간식" })
                        }
                      />
                    </CarouselItem>
                  ))}
